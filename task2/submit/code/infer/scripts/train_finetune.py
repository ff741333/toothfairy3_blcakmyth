...
def run(config_file: Optional[Union[str, Sequence[str]]] = None, **override):
    ...
    val_number = parser.get_parsed_content("val_number", default=-1)
    ...
    val_files = val_files[:val_number]
    if world_size > 1:
    ...
    with warnings.catch_warnings():
        ...
        train_ds = monai.data.PersistentDataset(
            data=train_files * num_epochs_per_validation, transform=train_transforms, cache_dir=os.path.join(data_file_base_dir,'cache')
        )
        val_ds = monai.data.Dataset(data=val_files, transform=val_transforms)
        test_ds = monai.data.PersistentDataset(data=test_files, transform=val_transforms, cache_dir=os.path.join(data_file_base_dir,'cache'))
            ...
            for _k in range(inputs_l.shape[0] // num_patches_per_iter):
                ...
                for click_indx in range(num_iters):
                    ...
                    for idx in range(len(prompt_class)):
                        ...
                        loss += loss_function(outputs[[idx]].float(), gt.float())
